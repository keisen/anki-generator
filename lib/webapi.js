...
const words = async (word)=> {
  ...
  if (res.status == 200 && !!res.body) {
    if (res.src == 'cache') {
      console.log(`The cache of wordsapi was found. Word: "${word}"`);
    } else {
      console.log(
        `Getting data from wordsapi succeeded!` +
        ` Word: "${word}", Limit: ${remaining}/${limit}`);
    }
  } else {
    console.warn(
      `Getting data from wordsapi failed!` +
      ` Word: "${word}", Limit: ${remaining}/${limit}` +
...
const twinword = async (word)=> {
  ...
  if (res.status == 200 && !!res.body && !!res.body.example) {
    if (res.src == 'cache') {
      ...
      console.log(
        `Getting data from twinword succeeded!` +
        ` Word: "${word}", Limit: ${remaining}/${limit}`);
    }
  } else {
    console.warn(
